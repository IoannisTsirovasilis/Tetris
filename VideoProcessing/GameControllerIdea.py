...
import math
import numpy as np
import VideoProcessing.GraphicsManagerIdea as GraphicsManager
...
BOARD_PIXELS = np.zeros(((BOARD_HEIGHT - 2) * ENLARGE_FACTOR, BOARD_WIDTH * ENLARGE_FACTOR), dtype=float)
...
def get_state():
    for i in range(BOARD_HEIGHT):
        for j in range(BOARD_WIDTH):
            for k in range(j * ENLARGE_FACTOR, ENLARGE_FACTOR * (j + 1)):
                if BOARD[i, j] == MAP_EMPTY:
                    BOARD_PIXELS[i, k] = MAP_EMPTY
                else:
                    BOARD_PIXELS[i, k] = MAP_BLOCK
    return BOARD_PIXELS
