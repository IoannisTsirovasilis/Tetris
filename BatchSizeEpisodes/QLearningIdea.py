...
from BatchSizeEpisodes import GameControllerIdea as GameController, TetrisQLearningIdea as Tetris
...
while True:  # Run until solved
    ...
    if episode_count % UPDATE_AFTER_EPISODES == 0:
        ...
        rewards_ranked = {}
        for r in range(len(episode_reward_history)):
            rewards_ranked[r] = episode_reward_history[r]

        keys = [k for k, v in sorted(rewards_ranked.items(), key=lambda item: item[1])]

        state_sample = np.array([np.array(episode_state)
                                .reshape(INPUT_SHAPE) for i in range(1, BATCH_EPISODES + 1)
                                 for episode_state in state_history[keys[-i]]])
        print(state_sample.shape)
        #sys.exit()
        state_next_sample = np.array([np.array(episode_next_state)
                                     .reshape(INPUT_SHAPE) for i in range(1, BATCH_EPISODES + 1)
                                      for episode_next_state in state_next_history[keys[-i]]])

        print("Indices:", indices)
        print("Number of sample states:", len(state_sample))

        rewards_sample = [reward for i in range(1, BATCH_EPISODES + 1) for reward in rewards_history[keys[-i]]]
        action_sample = [action for i in range(1, BATCH_EPISODES + 1) for action in action_history[keys[-i]]]
        done_sample = tf.convert_to_tensor(
            [float(history) for i in range(1, BATCH_EPISODES + 1) for history in done_history[keys[-i]]]
        )
