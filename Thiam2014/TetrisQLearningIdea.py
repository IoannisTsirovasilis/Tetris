...
from Thiam2014 import GameControllerIdea as GameController
import time
...
class Tetris:
    ...
    def get_state(self):
        col_heights = GameController.heights_diff()
        state = [3 if (col_heights[i] - col_heights[i + 1]) > 3 else -3 if (col_heights[i] - col_heights[i + 1]) < -3
        else col_heights[i] - col_heights[i + 1] for i in range(len(col_heights) - 1)]
        return state
